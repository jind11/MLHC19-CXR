...
from DatasetGenerator_TFIDF import DatasetGenerator
...
class ChexnetTrainer():
    ...
    def train (pathDirDataTrain, pathDirDataVal, pathFileTrain, pathFileVal, nnArchitecture, nnIsTrained, nnClassCount,
               ...
               feature_size, hidden_size_text, dump_dir):
        ...
        model_text = TFIDFRep(feature_size, hidden_size_text, text_rep_size).cuda()
        ...
        datasetTrain = DatasetGenerator(pathImageDirectory=pathDirDataTrain, pathDatasetFile=pathFileTrain, transform=transformSequence, fold='train')
        ...
        if pathDirDataVal != None:
            datasetVal =   DatasetGenerator(pathImageDirectory=pathDirDataVal, pathDatasetFile=pathFileVal, transform=transformSequence, fold='val')
        ...
        optimizer_text = optim.Adam(model_text.parameters(), lr=0.001, betas=(0.9, 0.999), eps=1e-08, weight_decay=1e-5)
    ...
    def epochTrain (model, model_text, dataLoader, optimizer, optimizer_text, scheduler, epochMax, classCount, loss):
        ...
        lossValNorm = 0
        losstensorMean = 0
        for batchID, (data, target, text_feats) in tqdm(enumerate(dataLoader)):
            ...
            text_rep = model_text(text_feats)
    ...
    def epochVal (model, model_text, dataLoader, loss):
        ...
        losstensorMean = 0
        with torch.no_grad():
            for i, (input, target, text_feats) in tqdm(enumerate (dataLoader)):
    ...
    def test (pathDirData, pathFileTest, pathModel, nnArchitecture, nnClassCount, nnIsTrained, trBatchSize, transResize, transCrop, launchTimeStamp):
        ...
        datasetTest = DatasetGenerator(pathImageDirectory=pathDirData, pathDatasetFile=pathFileTest, transform=transformSequence, fold='test')
        ...
        with torch.no_grad():
            for i, (input, target, text_feats) in enumerate(dataLoaderTest):
